...
public class EtudiantRepository(UniversiteDbContext context) : Repository<Etudiant>(context), IEtudiantRepository
{
    ...
    public async Task<List<Etudiant>> FindEtudiantsAvecNoteUeAsync(long idUe)
    {
        ArgumentNullException.ThrowIfNull(Context.Etudiants);
        return await Context.Etudiants
            .Where(e => e.ParcoursSuivi.UesEnseignees.Any(u => u.Id == idUe))
            .Include(e => e.NotesObtenues.Where(n => n.UeId == idUe))
            .ToListAsync();
    }
}